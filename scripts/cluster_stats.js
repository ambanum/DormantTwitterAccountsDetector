            ...
            options={
				...
				function select(arr) {
					return arr.filter((element, index) => {
						return index % (3*24) === 0; // One point a day
					});
				}

                let jsFile = 'window.TwitterUserGraph = {data: {}};';
                jsFile += `window.TwitterUserGraph.data.datesISO = ${JSON.stringify(select(dates).map((date) => date.toISOString()))};\n`
                jsFile += `window.TwitterUserGraph.data.nbAccountsSeries = ${JSON.stringify(select(nbAccountsSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followersMeanSeries = ${JSON.stringify(select(followersMeanSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followersMedianSeries = ${JSON.stringify(select(followersMedianSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followersFirstDecileSeries = ${JSON.stringify(select(followersFirstDecileSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followersLastDecileSeries = ${JSON.stringify(select(followersLastDecileSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followingsMeanSeries = ${JSON.stringify(select(followingsMeanSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followingsMedianSeries = ${JSON.stringify(select(followingsMedianSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followingsFirstDecileSeries = ${JSON.stringify(select(followingsFirstDecileSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.followingsLastDecileSeries = ${JSON.stringify(select(followingsLastDecileSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.tweetsMeanSeries = ${JSON.stringify(select(tweetsMeanSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.tweetsMedianSeries = ${JSON.stringify(select(tweetsMedianSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.tweetsFirstDecileSeries = ${JSON.stringify(select(tweetsFirstDecileSeries))};\n`
                jsFile += `window.TwitterUserGraph.data.tweetsLastDecileSeries = ${JSON.stringify(select(tweetsLastDecileSeries))};\n`
                jsFile += '\n';
